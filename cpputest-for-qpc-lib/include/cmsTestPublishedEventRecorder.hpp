/// @brief A class, using QActive, for subscribing to and recording published
///        events for unit testing purposes with the qpc framework.
/// @ingroup
/// @cond
///***************************************************************************
///
...
public:
    ...
    ~PublishedEventRecorder()
    {
        while (!m_recordedEvents.isEmpty()) {
            const auto e = m_recordedEvents.get();
            QF_gc(e);
        }
    };
    ...
    void recorderStart(uint_fast8_t priority)
    {
        ...
        for (enum_t sig = m_startingValue; sig < m_endValue; ++sig) {
            QActive_subscribe(getQActive(), sig);
        }
    }
